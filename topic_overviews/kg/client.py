...
"""
    ...
    def import_paper(
        ...
        (one statement each); ``generated_by`` an optional QID of the workflow or agent
        that produced the item (sets ``generated by`` P1642 — use ``M.Q_WORKFLOW``);
        ``tldr_model_qid`` an optional QID of the LLM that generated the TL;DR
        (written as a P1642 qualifier on the P1963 claim). All stored on the paper.
        """
        ...
        if tldr:
            if tldr_model_qid:
                qual = Qualifiers()
                qual.add(WBItem(prop_nr=M.P_GENERATED_BY, value=tldr_model_qid))
                item.add_claim(M.P_TLDR, value=tldr, qualifiers=qual)
            else:
